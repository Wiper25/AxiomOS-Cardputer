...
bool VoiceAssistant::Begin(const VoiceConfig& cfg) {
  ...
  audio_io_.Begin();  // config only, mic off

  began_ = true;
  // Off by default — no background mic/VAD until user enables in UI
  enabled_ = false;
  cfg_.enabled = false;
  SetStatus("voice off");
...
void VoiceAssistant::PttDown() {
  ...
  if (state_ == State::Thinking || state_ == State::Speak) {
    cancel_req_ = true;
    force_listen_ = true;  // re-arm after EnterIdle (must not clear force)
    return;
  }
  force_listen_ = true;
}
...
bool VoiceAssistant::EnsureWs() {
  ...
  // Connect() is async — wait until handshake finishes before SendTxt/PCM
...
void VoiceAssistant::EnterIdle() {
  ...
  FlushTxQueue();
  vad_.Reset();
  // Keep force_listen_ so Cancel+PttDown can re-arm in same session
  cancel_req_ = false;
  wake_req_ = false;
  ptt_held_ = false;
  ptt_release_ms_ = 0;
  if (!force_listen_) ptt_session_ = false;
  audio_io_.MicStop();  // free I2S for UI tones
  // Drop WS when idle — avoids ws.loop cost when not in a session
...
void VoiceAssistant::EnterListen(bool from_vad) {
  ...
  // Must announce listening BEFORE any PCM (server ignores BIN otherwise)
...
void VoiceAssistant::MicTask() {
  ...
  for (;;) {
    // Sleep hard when idle / disabled — no always-on VAD (was starving UI/WiFi)
    ...
    // PTT session: only max timeout ends mic here; hangover handled in Tick
...
void VoiceAssistant::SpkTask() {
  ...
  for (;;) {
    ...
    const size_t n = ws_.PopRx(buf, sizeof(buf));
    if (n >= sizeof(int16_t)) {
      audio_io_.PlayChunk(reinterpret_cast<const int16_t*>(buf), n / sizeof(int16_t));
      ++rx_chunks_;
      last_rx_ms_ = millis();
    } else {
      vTaskDelay(pdMS_TO_TICKS(5));
...
void VoiceAssistant::Tick() {
  ...
  // Idle + disabled: almost free
  if (!enabled_ && state_ == State::Idle) {
    return;
  }

  // Idle + enabled: light poll for ForceListen only (no ws.loop until session)
  ...
  if (state_ == State::Listen) {
    int16_t chunk[kChunkSamples];
    while (tx_queue_ && xQueueReceive(tx_queue_, chunk, 0) == pdTRUE) {
      if (ws_.SendBin(reinterpret_cast<const uint8_t*>(chunk), kChunkBytes)) {
        ++tx_chunks_;
      }
    }
    // PTT: after release, keep recording for kPttHangoverMs then end
